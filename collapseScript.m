...
for j = 1:2:8
    ...
    for i = 1:incr
        ...
        eval(['model = @ (t,z) ', genEqneasy(total)]);
        eval(geny0easy(total));
        options = odeset('NonNegative',eval(genList(total*2)));
        [t,y] = ode45(model,t,y0,options);
        if isempty(find(y(:,1)<1,1))
            t_c_1_big(i) = cap;
        else
            t_c_1_big(i) = find(y(:,1)<1,1);
            if t_c_1_big(i) > cap
                t_c_1_big(i) = cap;
            end
        end
